...
void EffectManager::InitializeCorn(float life, DirectX::SimpleMath::Vector3 pos, DirectX::SimpleMath::Vector3 dir)
{
	...
	for (std::list<MyEffect*>::iterator itr = m_effectList.begin(); itr != m_effectList.end(); itr++)
	{
		...
		Vector3 raff = Vector3(vel.y-0.5f, vel.x+0.5f, 0);
		raff *= sin(rand())*0.5f;
		vel = vel + raff;
...
void EffectManager::InitializeBut(float life, DirectX::SimpleMath::Vector3 pos, DirectX::SimpleMath::Vector3 dir)
{
	for (std::list<MyEffect*>::iterator itr = m_effectList.begin(); itr != m_effectList.end(); itr++)
	{
		Vector3 vel = dir;
...
void EffectManager::Render(bool selectFlag)
{

	for (auto itr = m_effectList.begin(); itr != m_effectList.end(); itr++)
	{
		(*itr)->Render(selectFlag);
	}
}
